...
function parsePagination(page, limit) {
  ...
  if (!Number.isInteger(parsedPage) || parsedPage < 1) {
    throw new TransactionsError("page must be a positive integer", 400);
  }

  if (!Number.isInteger(parsedLimit) || parsedLimit < 1 || parsedLimit > 100) {
    throw new TransactionsError("limit must be an integer between 1 and 100", 400);
  }
  ...
  return {
    async getUserTransactions({ userId, page, limit }) {
      if (!Number.isInteger(userId) || userId <= 0) {
        throw new TransactionsError("Authenticated user is required", 401);
      }
