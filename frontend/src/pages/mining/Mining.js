...
const Mining = () => {
    ...
    // Syncs Ledger balance with the backend for consistency
    ...
    const handleMine = async () => {
        ...
        try {
            ...
            // Success Logic: Increment local count and reset cooldown
            setSessionClicks(prev => prev + 1); 
            setCooldown(10); 
            fetchWalletInfo(); // Sync ledger immediately
        } catch (err) {
    ...
    return (
        <div className="dashboard-page"> {/* RESTORES DARK THEME */}
            
            {/* marginLeft: 0 ensures content centers without the sidebar gap */}
            <main className="main-content" style={{ marginLeft: 0, width: '100%', minHeight: '100vh' }}>
                
                <header className="header" style={{ textAlign: 'center', marginBottom: '40px' }}>
                    <h1>Mining Center</h1>
                </header>

                <div className="mining-container" style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 20px' }}>
                    
                    <div className="wallet-address-bar" style={{ marginBottom: '30px', opacity: 0.8, textAlign: 'center' }}>
                        <span>Active: </span>
